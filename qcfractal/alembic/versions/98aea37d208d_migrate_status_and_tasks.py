...
def upgrade():
    ...
    # If the base_result is incomplete but the task or service does not exist, then mark it as cancelled
    # op.execute("UPDATE base_result SET status = 'cancelled' WHERE status = 'incomplete'")

    # Now alter base_result to the new status enum, not that all the incompletes are removed
